...
for client in clients:
    try:
        ...
        if client.get('clientScreen') == 'EMBED':
            body['context']['thirdParty'] = {'embedUrl': 'https://www.google.com'}

        headers = {
            'Content-Type': 'application/json',
            'X-YouTube-Client-Name': '5' if client['clientName'] == 'IOS' else '3' if client['clientName'] == 'ANDROID' else '1' if client['clientName'] == 'WEB' else '85',
        ...
        formats = len(streamingData.get('formats', [])) + len(streamingData.get('adaptiveFormats', []))
        
        print(f"[{client['clientName']}] Status: {playability}, Formats: {formats}")
