...
export class DASHParser {
  static parse(content, manifestUrl) {
    ...
    for (const periodEl of mpd.querySelectorAll(':scope > Period')) {
      result.periods.push(DASHParser.parsePeriod(periodEl, baseUrl));
    }
    return result;
  }

  static parsePeriod(periodEl, parentBaseUrl) {
    ...
    for (const asEl of periodEl.querySelectorAll(':scope > AdaptationSet')) {
      adaptationSets.push(DASHParser.parseAdaptationSet(asEl, baseUrl));
    }
    return {
      id: periodEl.getAttribute('id'),
      duration: DASHParser.parseDuration(periodEl.getAttribute('duration')),
      adaptationSets,
  ...
  static parseAdaptationSet(asEl, parentBaseUrl) {
    ...
    const contentType = asEl.getAttribute('contentType') ||
      (mimeType.startsWith('video') ? 'video' : mimeType.startsWith('audio') ? 'audio' : 'unknown');
    ...
    for (const repEl of asEl.querySelectorAll(':scope > Representation')) {
      representations.push(DASHParser.parseRepresentation(repEl, baseUrl, sharedTemplate, contentType));
    }
  ...
  static parseRepresentation(repEl, parentBaseUrl, sharedTemplate, contentType) {
    ...
    if (segTemplate) {
      const r = DASHParser.buildSegmentsFromTemplate(segTemplate, id, bandwidth, baseUrl);
  ...
  static buildSegmentsFromTemplate(template, repId, bw, baseUrl) {
    const segments = [];
    let initUrl = null;
    ...
    if (template.timeline.length > 0) {
      ...
      for (let i = 0; i < 500; i++) {
        ...
        segments.push({ url: DASHParser.resolveUrl(DASHParser.sub(template.media, repId, bw, num, i * template.duration), baseUrl), duration: template.duration / template.timescale });
      }
    }
