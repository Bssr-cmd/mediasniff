...
export class TSToMP4Converter {
        ...
        if (pid === 0 && pmtPid === null) {
          ...
          if (p + 12 <= 188) {
            ...
            while (ep + 5 <= end && ep + 5 <= 188) {
              ...
              // 0x1B: H.264, 0x24: HEVC
              if ((streamType === 0x1B || streamType === 0x24) && videoPid === null) {
                videoPid = elemPid;
              } else if ([0x0F, 0x03, 0x04, 0x11, 0x81, 0x87].includes(streamType) && audioPid === null) {
                audioPid = elemPid;
              }
              ep += 5 + esInfoLen;
            }
      ...
      for (const seg of segArrays) {
        ...
        for (let i = sync; i + 188 <= len; i += 188) {
          ...
          if (offset + 4 <= 188 && seg[i + offset] === 0x00 && seg[i + offset + 1] === 0x00 && seg[i + offset + 2] === 0x01) {
            ...
            else if (audioPid === null && streamId >= 0xC0 && streamId <= 0xDF) audioPid = pid;
          }
        }
      }
    }
  ...
  /**
   * Builds the moov box. When baseMdatOffset > 0, populates stco with real offsets.
   */
  static buildMoov(videoData, audioData, baseMdatOffset = 0) {
    const rateIndices = [96000, 88200, 64000, 48000, 44100, 32000, 24000, 22050, 16000, 12000, 11025, 8000, 7350];
    ...
    if (videoData) {
      ...
      const avcCLen = 7 + spsLen + 1 + 2 + ppsLen + avcExtraLen;
      ...
      if (hasCtts) {
        ...
        cttsV = TSToMP4Converter.fullBox('ctts', 0, 0, cttsPayload);
      }
      ...
      // stco: populated directly with baseMdatOffset
      ...
      const stcoV = TSToMP4Converter.fullBox('stco', 0, 0, stcoPayload);
    ...
    if (audioData) {
      ...
      const stcoA = TSToMP4Converter.fullBox('stco', 0, 0, stcoPayload);
  ...
  static buildMP4Blob(videoData, audioData) {
    ...
    const dryMoov = TSToMP4Converter.buildMoov(videoData, audioData, 0);
    const baseMdatOffset = ftyp.length + dryMoov.length + mdatHeaderLen;

    // Pass 2: generate final moov with real chunk offsets populated directly in stco
    const moov = TSToMP4Converter.buildMoov(videoData, audioData, baseMdatOffset);
  ...
  static buildMP4(videoData, audioData) {
    ...
    const dryMoov = TSToMP4Converter.buildMoov(videoData, audioData, 0);
    const baseMdatOffset = ftyp.length + dryMoov.length + 8;
    const moov = TSToMP4Converter.buildMoov(videoData, audioData, baseMdatOffset);

    const mdatHeader = new Uint8Array(8);
    new DataView(mdatHeader.buffer).setUint32(0, 8 + mdatTotal);
    mdatHeader.set([0x6D, 0x64, 0x61, 0x74], 4);

    const totalLen = ftyp.length + moov.length + 8 + mdatTotal;
